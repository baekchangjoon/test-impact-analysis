...
- 작성일: 2026-06-13
- 상태: 설계 확정 → 계획 → **PoC 구현 완료**(단일 레포 직렬 수집 범위) (단위·E2E 33 + 실제 teamscale 에이전트/Docker 컨테이너 E2E + PIT mutation testing 모두 GREEN)
...
이 문서는 단일 레포 PoC부터 크로스 레포 MSA까지를 아우르는 **시스템 설계**다. 구현은 범위별 독립 spec→plan→구현 사이클로 진행하며, 현재는 단일 레포 PoC 범위가 구현되어 있다. 그 너머의 확장 로드맵(실패 분류기·정적 그래프 보강·크로스 레포)은 별도 기록으로 분리했다.
...
### 1.2 범위
- 단일 레포 PoC부터 크로스 레포 MSA까지를 하나의 설계로 다루되, 구현은 범위별 독립 사이클.
...
- 포맷: `.exec 호환` 또는 자체 포맷 + 변환기. **JaCoCo classId/probe 체계 유지** 필수.
  ...
  - **현재 실제 소비 산출물**: teamscale **testwise JSON**(에이전트가 per-test로 이미 분리해 산출)이며 위 `자체 포맷` 경로에 해당한다 — `.exec` OR-병합 우려는 **병렬 에이전트의 `.exec` 계약**에만 적용되고 현재 구현은 raw `.exec`를 직접 머지하지 않는다.
...
| 구현체 | 컨텍스트 분리 방식 | 상태 |
|--------|-------------------|------|
| `teamscale/jacoco 직렬` | reset→실행→dump 직렬 사이클 | 현재 주력 |
| `jacocoagent-parallel.jar` | ThreadLocal 스토어 + Baggage `test.id` (in-process) | 완성 시 **무중단 드롭인** (별도 프로젝트, 독립 일정) |

TIA는 인터페이스에만 의존하고 병렬 에이전트에 직접 의존하지 않는다. TIA는 직렬로 완주 가능하며, 병렬 에이전트는 준비되면 교체한다.
...
- `api_edges`(§5.2, §6.2)가 추가로 필요한 경우는 **오직**: ① 테스트가 없는(=커버리지 없는) 코드의 영향 전파, ② 방향성이 본질인 질의("서비스 A 변경 → B의 *어느 컨트롤러*가 영향?"). 이 경계 밖에서 co-coverage와 api_edges는 중복이므로, api_edges 구축은 이후 크로스 레포 확장에서 위 두 용도에 한정한다.
...
- **Java 8 레거시**:
  - 동적(JaCoCo 0.8.x)은 Java 8 런타임 지원 → 현재 구현부터 무비용 포함.
...
### 5.1 Baggage `test.id` — 필수 선결
RestAssured 클라이언트가 **모든 인입 요청에 `test.id` Baggage를 주입**한다. 현재 구현부터 의무.
...
### 6.3 저장 기술 — 단계적 채택
- **시작점: 레포당 SQLite + S3.** CodeGraph `.codegraph/` 패턴 차용. 나이틀리 잡이 `.exec` 가공 → `tia-{repo}-{commit}.sqlite` S3 업로드 → Action/CLI/MCP가 다운로드 후 로컬 쿼리. 인프라 신청 0, 운영 0, 스냅샷 자체가 불변 버전.
- **크로스 레포 본격화 시: 중앙 PostgreSQL.** api_edges 조인, "서비스 A 변경 → B의 테스트" 다중 레포 쿼리. 재귀 CTE로 그래프 탐색, 접근 제어·감사 확보.
...
- **Neo4j 등 그래프 DB**: 레포 10개·메소드 수만 개 규모에서 재귀 CTE로 충분. 탐색 깊이 10단계+ 상시화 전까지 과잉.
  - 단 [4-C]: 호출 그래프엔 재귀·상호호출 **사이클**이 있어 재귀 CTE에 명시적 사이클 방어가 필수다. **SQLite엔 `CYCLE` 절이 없으므로** 방문 노드를 경로에 누적해 차단하거나 깊이 상한을 둔다. PostgreSQL(크로스 레포 확장 시)은 `CYCLE` 절 사용.
...
현재 구현된 범위는 **단일 레포 PoC**다: teamscale-jacoco-agent per-test 매핑 + git diff 교차 →
영향 테스트 선별, **플레이키 비율 선측정 포함**. `test.id` Baggage는 **클라이언트 주입 + dump
라벨만**(크로스 서비스 전파는 이후 확장) [4-D]. 소요 약 2주(20~25h).

그 너머의 확장 로드맵(실패 분류기·정적 그래프 보강·크로스 레포 API 경계/Kafka 귀속)과 단계별 추정은
저장소에서 분리해 별도 기록으로 보관한다.
...
- **플레이키 테스트가 판정 신뢰도를 잠식**: "diff 무관 실패 = 인프라" 판정의 ROI는 스위트 안정성에 종속 → PoC 단계에서 플레이키 비율 선측정.
...
| D9 | `test.id` Baggage = 현재 구현 필수 선결 | 병렬 에이전트 입력 와이어 사전 연결 |
...
- 인스턴스 분리 절충안의 recall 손실 정량화 [8.7] — 이후 측정.
- Roaring Bitmap 역방향 쿼리("이 라인 커버하는 테스트") 보완 인덱스 [9.7.2] — 필요 시 추가.
- 병렬 에이전트 정밀도/컨텍스트 전파/카나리 [병렬 doc Q1~Q6] — 병렬 에이전트 프로젝트 책임, TIA PoC 데이터로 검증.
