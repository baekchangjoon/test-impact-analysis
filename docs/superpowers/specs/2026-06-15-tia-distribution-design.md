...
- 작성일: 2026-06-15
- 상태: 설계 초안 → 멀티모델 리뷰(Claude·Gemini·GPT) 반영 → Phase별 구현
...
- **D3.1 에이전트 와이어링(최대 리스크):** 플러그인은 (a) 커버리지 에이전트 jar를 전용 Gradle configuration으로 해소(자동 다운로드는 §5.3 정책), (b) `Test` 태스크의 JVM fork에 `-javaagent:<agentJar>=…`를 주입 — **control-port는 fork마다 동적 할당**한다(`maxParallelForks>1`에서 데모의 고정 6310을 그대로 쓰면 `BindException` 충돌; 포트 0/free-port 할당 또는 소켓·파일 기반 통신), (c) per-test dump 산출물(`.exec` 또는 testwise JSON)을 수확. teamscale/parallel 차이는 §5.3의 어댑터 경계로 흡수한다.
