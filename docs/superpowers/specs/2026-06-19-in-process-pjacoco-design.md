...
## 6.4 graph-rag 피드백 추가 반영 (P2-5 / P3-4)
- **P2-5 (빌드캐시 무력화 방지, REQ-010)**: agent `.exec`는 `-javaagent` side-effect라 테스트 태스크가
  빌드 캐시(FROM-CACHE)/up-to-date로 복원되면 실제 실행되지 않아 빈 커버리지가 조용히 생긴다. in-process
  수집 invocation에 `--no-build-cache`를 추가하고(태스크는 이미 `outputs.upToDateWhen{false}`),
  오케스트레이터의 모드별 `.exec` 0개 가드(이미 존재)로 빈 testwise를 명시 실패시킨다.
- **P3-4 (인덱스 저장 위치 가이드, REQ-011, 문서만)**: `GETTING-STARTED`에 "tia.db를 git에 커밋하지
  말고 공유 비추적 위치(git common dir 또는 `$XDG_CACHE_HOME`)에 두라"는 짧은 절 추가. CLI `--db`
  자동 기본값은 후속 코드 작업으로 보류.
- 보류(후속 phase, 본 spec 밖): **P3-3**(out-of-process 자기코드 멀티프로세스 수집), **P1-3**(index
  마지막 build만 보임 — `CoverageStore.load` LIMIT 1 경고/병합).
