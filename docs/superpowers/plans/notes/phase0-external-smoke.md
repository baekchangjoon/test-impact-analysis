...
7. 영향 분석: `tia impact --db tia-<레포>.db --commit <인덱싱커밋>` (--git-ref 미지정 시 베이스=인덱싱커밋 → 라인 공간 정렬 [§6.2 4-B]. **diff 베이스가 인덱싱 커밋과 달라지면 비트 AND 무효** — Phase 0는 라인 재조정 미구현)
...
- [ ] 신규 .java 추가 → "보수적 전체 선택" (영향 불명, 정적 폴백 Phase 2)
...
- 커버리지는 인덱싱 커밋 기준 → staleness는 응답의 "매핑 기준 커밋"으로 노출 (§6.5), 라인 재조정은 Phase 1+ ([4-B])
- 노이즈 베이스라인 차감(§3.4)은 Phase 0 미구현 — 관찰만
