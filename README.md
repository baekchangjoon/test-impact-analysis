...
- **동적 우선.** Spring 프록시/AOP/리플렉션/비동기는 정적 호출그래프를 흔듭니다. 그래서 "테스트 T가 실제로 실행하며 밟은 라인"을 동적으로 수집해 1급 신호로 씁니다. 정적 분석은 보조(이후 Phase).
...
- **커밋이 1급 시민.** 커버리지 매핑은 특정 빌드 스냅샷이라, 인덱싱 커밋과 diff의 라인 공간이 일치할 때만 비트 AND가 유효합니다. Phase 0는 "diff 베이스 = 인덱싱 커밋"을 불변식으로 둡니다.
