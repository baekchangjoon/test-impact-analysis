...
> 빌드/CI가 외부에서 가져다 쓰는 커버리지 에이전트(parallel-per-test-coverage / pjacoco,
> `io.github.beltian.pjacoco:*:2.0.0` — Maven Central)는 TIA가 **번들하지 않으며**(사용자 제공 또는
> Central 다운로드/소스 빌드, 설계 §5.3) 본 고지의 대상이 아닙니다.
